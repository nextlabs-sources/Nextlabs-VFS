...
/**
 * Provides an implementation for NTLMv1, NTLMv2, and NTLM2 Session forms of the
...
final class NTLMEngineImpl implements NTLMEngine {
	...
	protected static final int FLAG_REQUEST_SEAL = 0x00000020; // Request key exchange for message confidentiality in NEGOTIATE message. MUST
																// be used in conjunction with 56BIT.
	...
	protected static final int FLAG_REQUEST_ALWAYS_SIGN = 0x00008000; // Requests a signature block on all messages. Overridden by REQUEST_SIGN and
																		// REQUEST_SEAL.
	 ...
	 * includes all the information for authentication, host, domain, username and
	 * the result of encrypting the nonce sent by the server using the user's
	 * password as the key.
	 *
	 ...
	 *
	 * @return The NTLM2 Session Response. This is placed in the NTLM response field
	 *         of the Type 3 message; the LM response field contains the client
	 *         challenge, null-padded to 24 bytes.
	 */
	 ...
	 * @param password The password.
	 *
	 * @return The NTLM Hash of the given password, used in the calculation of the
	 *         NTLM Response and the NTLMv2 and LMv2 Hashes.
	 */
	...
	static class NTLMMessage {
		...
		NTLMMessage(final String messageBody, final int expectedType) throws NTLMEngineException {
			messageContents = Base64.getDecoder().decode(EncodingUtils.getBytes(messageBody, DEFAULT_CHARSET));
		 ...
		 *
		 * @param maxlength the maximum length of the response to prepare, not including
		 *                  the type and the signature (which this method adds).
		 */
	...
	static class Type1Message extends NTLMMessage {
		...
		Type1Message(final String domain, final String host) throws NTLMEngineException {
			...
			try {
				...
				domainBytes = unqualifiedDomain != null ? unqualifiedDomain.toUpperCase(Locale.ENGLISH).getBytes("ASCII") : null;
			} catch (final UnsupportedEncodingException e) {
	...
	static class Type2Message extends NTLMMessage {
		...
		Type2Message(final String message) throws NTLMEngineException {
			...
			// Next 8 bytes, starting at offset 12: target field (2 ushort lengths, 1 ulong
			// offset)
			...
			// Next 8 bytes, starting at offset 40: targetinfo field (2 ushort lengths, 1
			// ulong offset)
	...
	static class Type3Message extends NTLMMessage {
		...
		Type3Message(final String domain, final String host, final String user, final String password, final byte[] nonce, final int type2Flags, final String target, final byte[] targetInformation) throws NTLMEngineException {
			...
			byte[] userSessionKey;
			try {
				// This conditional may not work on Windows Server 2008 R2 and above, where it
				// has not yet
				// been tested
			...
			try {
				...
				domainBytes = unqualifiedDomain != null ? unqualifiedDomain.toUpperCase(Locale.ENGLISH).getBytes("UnicodeLittleUnmarked") : null;
		...
		String getResponse() {
			...
			addULong(
					...
					// Required flags
					(type2Flags & FLAG_REQUEST_LAN_MANAGER_KEY) | (type2Flags & FLAG_REQUEST_NTLMv1) | (type2Flags & FLAG_REQUEST_NTLM2_SESSION) |

					// Protocol version request
							FLAG_REQUEST_VERSION |

							// Recommended privacy settings
							(type2Flags & FLAG_REQUEST_ALWAYS_SIGN) | (type2Flags & FLAG_REQUEST_SEAL) | (type2Flags & FLAG_REQUEST_SIGN) |

							// These must be set according to documentation, based on use of SEAL above
							(type2Flags & FLAG_REQUEST_128BIT_KEY_EXCH) | (type2Flags & FLAG_REQUEST_56BIT_ENCRYPTION) | (type2Flags & FLAG_REQUEST_EXPLICIT_KEY_EXCH) |

							(type2Flags & FLAG_TARGETINFO_PRESENT) | (type2Flags & FLAG_REQUEST_UNICODE_ENCODING) | (type2Flags & FLAG_REQUEST_TARGET));
	...
	/**
	 * Cryptography support - MD4. The following class was based loosely on the RFC
	 * and on code found at http://www.cs.umd.edu/~harry/jotp/src/md.java. Code
	 * correctness was verified by looking at MD4.java from the jcifs library
	 * (http://jcifs.samba.org). It was massaged extensively to the final form found
	 * here by Karl Wright (kwright@metacarta.com).
	 */
	static class MD4 {
